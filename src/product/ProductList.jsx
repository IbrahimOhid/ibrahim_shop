...
const ProductList = () => {
  ...
  const handleSearchProduct = (value) => {
    if (!value.trim()) {
      return setDisplayProduct(allProduct);
    }
    const newSearchProduct = allProduct.filter((item) =>
      item.name.toLowerCase().includes(value.toLowerCase())
    );
    setDisplayProduct(newSearchProduct);
  };

  return (
    <div>
      <Header onSearch={handleSearchProduct} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-6">
              ...
              <SortProduct onSort={handleSortProduct} />
            </div>
            {/* Products Grid */}
